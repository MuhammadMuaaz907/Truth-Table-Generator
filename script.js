...
document.getElementById('truth-table-form').addEventListener('submit', function(event) {
...
function generateTruthTable(variables, expression) {
    ...
    combinations.forEach(combination => {
        ...
        combination.forEach(value => {
            ...
            td.textContent = value; // Keep the values as true/false
            row.appendChild(td);
        });

        const resultTd = document.createElement('td');
        const result = evaluatePostfix(postfixExpression, variables, combination);
        resultTd.textContent = result ? 1 : 0; // Display the result as 0/1
